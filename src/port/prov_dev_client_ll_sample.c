...
int prov_dev_client_ll_sample_run(const char * global_prov_uri , const char * id_scope)
{
    ...
    else
    {
        ...
        else
        {
            do
            {
                ...
                vTaskDelay(user_ctx.sleep_time/portTICK_PERIOD_MS);
    ...
    strcpy(azureiot , user_ctx.iothub_uri );
    ...
    if (strcmp(azureiot , verifyresult) == 0)
    {
    printf("Azure IoT URL provisioning complete.\n");
    }
    else
    {
        printf("error comparing Azure IoT URL\n");
    }
